...
export default function FriendsPage() {
  ...
  const fetchActivities = async () => {
    setLoading(true)
    try {
      const res = await fetch('http://localhost:8081/api/friends')
  ...
  const handleJoin = async (activityId: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:8081/api/friends/${activityId}/join?participantName=${encodeURIComponent(participantName)}&participantContact=${encodeURIComponent(participantContact)}`, {
        method: 'POST'
      })
      if (!res.ok) {
